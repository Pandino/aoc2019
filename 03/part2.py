...
for a_move in wire_a_moves:
    ...
    for b_move in wire_b_moves:
        ...
        if c:
            total_a = len_a + distance(c, a)
            total_b = len_b + distance(c, b)
            intersections.append((c, (total_a, total_b)))
        len_b += distance(b, b_next)
        b = b_next
    len_a += distance(a, a_next)
    a = a_next
    b = (0, 0)
    len_b = 0
...
distances = list()
for cross, lengths in intersections:
    d = distance((0, 0), cross)
    length = sum(lengths)
    print(f'P={cross} distance={d} length={length}')
    distances.append((d, length))
print(f'CLosest intersection: {min(distances, key=lambda distance: distance[1])}')
